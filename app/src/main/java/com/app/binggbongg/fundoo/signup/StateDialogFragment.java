...
public class StateDialogFragment extends DialogFragment {
    ...
    ArrayList<GetState.Results> stateList = new ArrayList<>();
    ...
    private void initView() {
        if (SignupActivity.stateList.size() > 0) {
            stateList.addAll(SignupActivity.stateList);
            countryAdapter = new CountryAdapter(getContext(), stateList);
    ...
    public class CountryAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        private ArrayList<GetState.Results> coutryList;


        public CountryAdapter(Context context, ArrayList<GetState.Results> professionList) {
        ...
        public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
            if (holder instanceof MyViewHolder) {
                ((MyViewHolder) holder).countryName.setText(coutryList.
                        get(position).getStateName());

                if (countryName.equals(coutryList.get(position).getStateName())) {
                ...
                ((MyViewHolder) holder).countryName.setOnClickListener(v -> {
                    ...
                    bundle.putString("state_name", coutryList.get(position).getStateName());
                    bundle.putString("state_id", coutryList.get(position).getStateId());
