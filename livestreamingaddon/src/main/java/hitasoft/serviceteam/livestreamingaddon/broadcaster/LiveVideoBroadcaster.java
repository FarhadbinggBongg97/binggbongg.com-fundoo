package hitasoft.serviceteam.livestreamingaddon.broadcaster;
...
import hitasoft.serviceteam.livestreamingaddon.broadcaster.encoder.AudioHandler;
import hitasoft.serviceteam.livestreamingaddon.broadcaster.encoder.CameraSurfaceRenderer;
import hitasoft.serviceteam.livestreamingaddon.broadcaster.encoder.TextureMovieEncoder;
import hitasoft.serviceteam.livestreamingaddon.broadcaster.encoder.VideoEncoderCore;
import hitasoft.serviceteam.livestreamingaddon.broadcaster.network.IMediaMuxer;
import hitasoft.serviceteam.livestreamingaddon.broadcaster.network.RTMPStreamer;
import hitasoft.serviceteam.livestreamingaddon.broadcaster.livebroadcasterutils.Resolution;
import hitasoft.serviceteam.livestreamingaddon.broadcaster.livebroadcasterutils.LiveBroadCastUtils;
...
public class LiveVideoBroadcaster extends Service implements ILiveVideoBroadcaster, CameraHandler.ICameraViewer, SurfaceTexture.OnFrameAvailableListener {
    ...
    public boolean startBroadcasting(String rtmpUrl) {
        ...
        if (LiveBroadCastUtils.doesEncoderWorks(context) != LiveBroadCastUtils.ENCODER_WORKS) {
                ...
                try {
                    ...
                    if (sCameraProxy.isCameraAvailable()) {
                        ...
                        if (parameters != null) {
                            ...
                            if (LiveBroadCastUtils.doesEncoderWorks(context) == LiveBroadCastUtils.ENCODER_NOT_TESTED) {
                                boolean encoderWorks = VideoEncoderCore.doesEncoderWork(previewSize.width, previewSize.height, 300000, 20);
                                LiveBroadCastUtils.setEncoderWorks(context, encoderWorks);
                            }
                        }
                    } else {
                        sCameraProxy = null;
    ...
    private int setCameraParameters(Camera.Parameters parameters) {
        ...
        /** Get Preview size method: 2 - Optimal for maximum devices*/
        optimalSize = getOptimalPreviewSize(previewSizeList, preferredWidth, preferredHeight);
        ...
        this.previewSize = new Resolution(cameraSize.width, cameraSize.height);

        return choosenPreviewsSizeList.size();
    }
    ...
    private Camera.Size getOptimalPreviewSize(List<Camera.Size> sizes, int w, int h) {
        ...
        for (Camera.Size size : sizes) {
            ...
            if (Math.abs(size.height - targetHeight) < minDiff) {
                choosenPreviewsSizeList.add(new Resolution(size.width, size.height));
                optimalSize = size;
